...
docker run --rm -t  -e "AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID" -e "AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY" -e "AWS_DEFAULT_REGION=$SC_AWS_REGION" -e "SC_AWS_S3_BUCKET=$SC_AWS_S3_BUCKET" -e "SC_ENV=$SC_ENV" -e "BACKUP_DIR=$BACKUP_DIR" -v "/tmp/sc/backup:/tmp/sc/backup" mesosphere/aws-cli s3 cp $BACKUP_DIR/$TIMESTAMP-squirrelcart-hh.tar.gz s3://$SC_AWS_S3_BUCKET/backup/$TIMESTAMP/$TIMESTAMP-squirrelcart-hh.tar.gz

docker run --rm -t  -e "AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID" -e "AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY" -e "AWS_DEFAULT_REGION=$SC_AWS_REGION" -e "SC_AWS_S3_BUCKET=$SC_AWS_S3_BUCKET" -e "SC_ENV=$SC_ENV" -e "BACKUP_DIR=$BACKUP_DIR" -v "/tmp/sc/backup:/tmp/sc/backup" mesosphere/aws-cli s3 cp $BACKUP_DIR/$TIMESTAMP-squirrelcart-hh.sql.gz s3://$SC_AWS_S3_BUCKET/backup/$TIMESTAMP/$TIMESTAMP-squirrelcart-hh.sql.gz
#rm -rf $BACKUP_DIR
